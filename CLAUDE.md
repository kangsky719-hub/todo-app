...
2. **간트차트 뷰 (노션식 타임라인)**:
   - 일자별 칸 그리드(기본 32px), 월/일 헤더, 주말 음영, 오늘 빨간 원 표시
   - **줌 조절**: 범례 우측 −/+ 버튼으로 칸 너비 5단계(16/24/32/44/60px, GANTT_ZOOMS), localStorage `ganttZoom`에 저장, 줌 변경 시 보던 위치 유지(스크롤 비율 환산). CSS는 `--gantt-cell-w` 변수 사용
   - **막대 업무명 표시**: 막대 너비 56px 이상이면 안쪽 흰 글씨(ellipsis), 좁으면 오른쪽 바깥 회색 글씨(.gantt-bar-label.outside, max-width 180px). pointer-events:none이라 드래그 방해 없음
...
9. **자연어 빠른 추가**: 상단 입력줄에 "다음주 화요일 라인점검 3일간 #제조 긴급" 식으로 입력하면 자동 인식, 실시간 미리보기 후 Enter로 추가. 기존 상세 폼은 details로 접힘. 파서(parseQuickInput/resolveDateToken/resolveDuration)가 인식하는 것:
   - **날짜**: 오늘/내일/모레/글피, N일·N주·N개월 후, (이번주/다음주/다다음주) X요일, 이번달·다음달 초/말, M월 D일·M/D (한 달 넘게 지난 M/D만 내년)
   - **소요 기간**: N일/N주/N개월(간·동안·짜리·걸리는), 한글(하루~열흘·보름·일주일·한달 등) → 시작일+기간으로 종료일 계산
   - **날짜 범위**: "A~B", "A부터 B까지"(종료 요일은 시작일 기준 해석), 양쪽이 모두 날짜일 때만 분리(제목의 '에서/부터' 보호)
   - **#프로젝트, !우선순위**, 자연어 우선순위(긴급·급함·중요→높음, 사소·천천히→낮음), 반복(매일/매주/매월, "매주 화요일"도 인식)
...
18. **업무 노트 탭 (일일 업무일지, 제약공장 특화)**: 날짜별 노트 페이지(◀오늘▶ 이동 + date picker), 큰 날짜 히어로(일자·요일·상대날짜·주차), 그날 걸친 업무 자동 스트립(칩 클릭→목록 수정폼), GMP 현장 템플릿 5종(인수인계/생산일지/회의록/이슈기록/현장순회) 원클릭 삽입, 600ms 디바운스 자동저장, **선택 문장→할 일 변환**(parseQuickInput 재사용, 날짜 없으면 노트 날짜, memo에 출처 기록), 지난 노트 목록(최근 30개)+내용 검색, 인쇄(#note-print-area만 @media print로 출력). **localStorage `workNotes`{date:text}에만 저장 — 클라우드 전송 안 함(사내 정보 보호)**. 내보내기가 신형식 {app, exportedAt, todos, workNotes, approvalLogs}로 변경됨, 가져오기는 구형식(배열)도 호환
...
5. **캐시 버스팅**: index.html에서 `style.css?v=N`, `script.js?v=N` — **파일 수정 시 반드시 v 번호를 올려야 함** (현재 v=20). netlify.toml에 no-cache 헤더 설정됨
...
- 캐시 버전은 index.html에서 `?v=N`으로 관리 — 파일 수정 시 반드시 올릴 것 (현재 v=20). sw.js의 CACHE 이름·CORE 목록도 함께 갱신. GitHub Pages는 10분 캐시라 버전 갱신 필수
